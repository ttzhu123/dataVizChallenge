...
**Answer:** The dataset contains **2,430 games in 2010** and **2,429 games in 2021** (4,859 games in total). 
We aggregate individual games into stadium averages for several reasons: (1) **Unit of analysis** — the question is “which stadium is most run-friendly?”, so we need one summary per stadium, not per game. (2) **Reduce noise** — single games are highly variable (e.g., blowouts vs. pitcher’s duels); averaging over a full season gives a stable estimate of each ballpark’s run environment. (3) **Visualization clarity** — plotting thousands of game-level points would be unreadable; summarizing to roughly 30 stadiums (one per team) makes bar charts or similar plots interpretable. (4) **Fair comparison** — stadiums host different numbers of games; using *average* runs per game puts every ballpark on the same scale for comparison.
:::
